...
def SYK_E_from_job(job,with_prerotations=False,readout_mitigate=False,part=None,uncorrelated=False):
    tags = job.tags()
    ...
    if with_prerotations:
        starting_index = -2*(reset_circuits+1)*num_measurements+1
    else:
        starting_index = -(reset_circuits+1)*num_measurements+1
    ...
    if readout_mitigate:
        E = part.measure_H(counts,terms_measured,paulis,coeffs,uncorrelated)
    else:
...
class readout_partition:
    ...
    def measure_pauli(self,qubits_in_pauli,counts,R_inv_all=[]):
        ...
        from numpy.linalg import inv
        islands = self.get_islands()
        which_islands = [ [q in island for island in islands].index(True) for q in qubits_in_pauli] # list of which island each relevant qubit is in
        ...
        for outcome_str in counts:
            ...
            for I in which_islands:
                ...
                for j in range(2**n_I):
                    ...
                    P_J = 1 - 2*(sum( 1 for q in range(n_I) if output_bitstr_I[-1-q] == '1' and islands[I][q] in qubits_in_pauli)%2)
                    #print('Z = '+str(P_J))
                    PR_I += P_J * R_inv_all[I][j,i_I]
                prod = prod * PR_I
            # print('prod for '+str(outcome_str)+' = '+str(prod))
            E += p_i * prod
    ...
    def measure_H(self,counts,which_paulis_in_each,paulis,coeffs,uncorrelated=False,readout_mitigate=True):
        ...
        for i in range(len(counts)):
            for t in which_paulis_in_each[i]:
                E += coeffs[t] * self.measure_pauli( np.nonzero(paulis[t])[0], counts[i], Rinv_all)
